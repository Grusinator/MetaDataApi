...
from MetaDataApi.dataproviders.services.fetch_data_from_provider import fetch_data_from_provider_endpoint
...
def handle_store_data(endpoint_name, provider_name, request):
    if request.GET.get('store_data', False):
        try:
            fetch_data_from_provider_endpoint(provider_name, endpoint_name, request.user.pk)
