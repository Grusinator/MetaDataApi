...
"""
    ...
    command.run(f"bandit -r {source_path}", echo=True)
...
"""
    ...
    command.run("black . --line-length 120", echo=False)


@task
def isort(command, color=False):
...
@task
def pytest(command, test_files=".", test_results="test_results", open_results=True, skip_integration=True):
    ...
    Keyword Arguments:
        ...
        integrationtest {bool} -- If True tests marked integrationtest or functionaltest will be run.
            Otherwise not. (default: {False})
            These tests requires the test backend server running.
        test_results {string} -- If not None test reports will be generated in the test_results folder
        open_results {bool} -- If True test reports in the 'test_results' folder will be opened in a browser
...
"""
    ...
    command_string = f"pytest {test_files} --doctest-modules -v"
    if test_results:
        command_string += f" --junitxml=tests/{test_results}/test-results-docs.xml"
        # command_string += f" --cov-report html:{test_results}/cov_html"

    if skip_integration:
        command_string += f" -m \"not integration\""
...
@task()
def pylint(command, files="source/conf.py tasks scripts tests"):
...
@task
def mypy(command, files="source/conf.py tasks tests"):
